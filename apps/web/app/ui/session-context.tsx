...
type SessionPhase = "checking" | "disconnected" | "signing" | "connected" | "reconnecting" | "error";
  ...
  useEffect(()=>{
    ...
    const probe=async()=>{
      try{
        ...
        if(cancelled)return;
        if(response.status===401){setSession(null);setOrganizations([]);setPhase("disconnected");setError("");return}
        if(!response.ok)throw new Error(`Session probe failed (${response.status})`);
        if(phase!=="connected"&&phase!=="signing"){
          const next=await response.json() as AuthSession;
          const organizationResponse=await fetch(`${apiBase}/organizations`,{credentials:"include",cache:"no-store"});
          if(!organizationResponse.ok)throw new Error(`Organization recovery failed (${organizationResponse.status})`);
          const recoveredOrganizations=((await organizationResponse.json()) as {items:Organization[]}).items;
          if(cancelled)return;
          setSession(next);setOrganizations(recoveredOrganizations);setPhase("connected");setError("");
        }
      }catch{if(!cancelled&&phase!=="signing"){setPhase("reconnecting");setError("AEOS API is temporarily unavailable. Read-only recovery is retrying automatically.")}}
      finally{if(!cancelled)timer=window.setTimeout(probe,2500)}
    };
    void probe();
