  ...
  async run(input:FrozenAdvisoryInput){
    ...
    for(let attempt=1;attempt<=attempts;attempt++){
      ...
      try{
        ...
        validateDecisionOutput(output,[...input.allowedEvidenceIds],allowedKnowledgeCitations({schemaVersion:"decision.retrieval-bundle.v1",manifests:[...input.retrievalManifests],bundleHash:"not-used-by-citation-projection",assetExecutionAuthorized:false}),Object.fromEntries(input.retrievalManifests.map(manifest=>[manifest.role,{manifestHash:manifest.manifestHash,status:manifest.status}])));
        return output;
      }catch(error){
