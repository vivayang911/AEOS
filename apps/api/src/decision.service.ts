...
export class DecisionService implements OnModuleInit {
  ...
  private validateProviderOutput(output:unknown,input:FrozenAdvisoryInput){try{validateDecisionOutput(output,[...input.allowedEvidenceIds],allowedKnowledgeCitations({schemaVersion:"decision.retrieval-bundle.v1",manifests:[...input.retrievalManifests],bundleHash:hash({schemaVersion:"decision.retrieval-bundle.v1",manifests:[...input.retrievalManifests],assetExecutionAuthorized:false}),assetExecutionAuthorized:false}),Object.fromEntries(input.retrievalManifests.map(manifest=>[manifest.role,{manifestHash:manifest.manifestHash,status:manifest.status}])));return output}catch(error){if(error instanceof DecisionOutputValidationError)throw new BadRequestException(error.message);throw error}}
