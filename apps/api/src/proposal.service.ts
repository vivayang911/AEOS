...
export class ProposalService {
  ...
  async list(org: string) { const result = await this.db.query("SELECT p.*,coalesce((SELECT o.state FROM proposal_state_observations o WHERE o.organization_id=p.organization_id AND o.proposal_id=p.id ORDER BY o.ordinal DESC LIMIT 1),p.state) AS effective_state FROM proposals p WHERE p.organization_id=$1 ORDER BY p.created_at DESC,p.id DESC LIMIT 100", [org]); return { items: result.rows.map((row) => this.map(row)) }; }
  async get(org: string, proposalId: string) { const result = await this.db.query("SELECT p.*,coalesce((SELECT o.state FROM proposal_state_observations o WHERE o.organization_id=p.organization_id AND o.proposal_id=p.id ORDER BY o.ordinal DESC LIMIT 1),p.state) AS effective_state FROM proposals p WHERE p.organization_id=$1 AND p.id=$2", [org, proposalId]); if (!result.rowCount) throw new NotFoundException("Proposal not found"); return this.map(result.rows[0]); }
