...
async function main() {
  ...
  const deployment = buildBalanceObserverDeploymentPlan({ chainId: Number(network.chainId), reporter: REPORTER, creationBytecode: artifact.bytecode?.object, runtimeBytecode: artifact.deployedBytecode?.object, reporterImmutableReferences: Object.values(artifact.deployedBytecode?.immutableReferences || {}).flat(), artifactCompiler: artifact.metadata?.compiler?.version || "unknown", artifactSource: "contracts/src/AEOSBalanceObserver.sol" });
  ...
  const deploymentTx = transaction(1, pendingNonce, "DEPLOY_AEOS_BALANCE_OBSERVER", null, deployment.unsignedTransaction.data, { predictedAddress, initCodeHash: deployment.unsignedTransaction.initCodeHash, expectedRuntimeBytecodeHash: deployment.artifact.runtimeBytecodeHash });
