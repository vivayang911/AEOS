...
| Prototype demo video | The earlier 170-second v3 candidate was rejected after human review because variable time compression altered narration speed/pitch and its level was excessive. A replacement 180-second, 1920×1080 v4 uses native-rate narration and 0.42 gain; human acceptance, strict H.264/AAC confirmation and a hosted URL remain pending | **Revised local candidate / publication pending** | Complete normal-speed review, verify codecs on a machine with `ffprobe`, then publish only the accepted v4 file |
...
> Superseded status notice (2026-08-26): the path described above as not yet live subsequently completed one bounded 11-step sample. The paragraph is retained as a dated checkpoint. The public remote refresh and clean-clone check subsequently passed at `58948cf...d52f`. A narrated local video candidate and exact judge-local route now exist. Current submission blockers are normal-speed video acceptance, hosted Deck/video URLs, final public-repository refresh, DoraHacks submission and written resolution of the deadline conflict—not the original source observation, ASC anchor or governance outcome.
