...
| Public GitHub with README | [`vivayang911/AEOS`](https://github.com/vivayang911/AEOS) is public; local and remote `main` matched final-content commit `3e7b374...31846`, the Secret Scan passed, and `.tmp/`, `LOCAL-MANUALS/` and the video were excluded | **Pass / remote commit verified** | Run a fresh clean-clone PRD check against this exact final-content commit |
...
| Prototype demo video | The earlier 170-second v3 was rejected because variable time compression altered narration speed/pitch and its level was excessive. The replacement 180-second, 1920×1080 v4 uses native-rate narration and 0.42 gain and passed normal-speed human acceptance; strict H.264/AAC confirmation and a hosted URL remain pending | **Human accepted / publication pending** | Verify codecs on a machine with `ffprobe`, then publish only the accepted v4 file |
...
> Superseded status notice (2026-08-26): the path described above as not yet live subsequently completed one bounded 11-step sample. The paragraph is retained as a dated checkpoint. The earlier clean-clone check passed at `58948cf...d52f`; final-content `main` was later published at `3e7b374...31846`. The v4 video passed normal-speed human acceptance and the deadline conflict is resolved. Current blockers are hosted video, final clean-clone verification and DoraHacks draft/final submission—not the original source observation, ASC anchor or governance outcome.
