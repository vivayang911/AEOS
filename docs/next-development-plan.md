...
15. `LOCAL_VERIFIED / EXTERNAL_PENDING`: live source-chain discovery now reads Creditcoin ChainInfo and refuses unsupported/unattested Sepolia inputs. Configure the official Proof Builder hostname plus a separately approved Sepolia RPC. A positive DAO-finality sample must wait for an AEOS-related deployed Governor and real proposal; never substitute an unrelated proposal or Mock terminal state.
